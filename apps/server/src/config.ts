 ...
 * records, and per-space E2EE keyrings. Both OctoChat and OctoVault clients point
 * their `spacesNamespace` here so they share one space registry. Content collections
 * (objindex, objpub, objinv, obj*, objectindex, etc.) live in each app's own
 * namespace — not here.
 *
 *   spaces        — per-identity space list.
 *   spaceregistry — per-space access record `{ owner, members, name, image }`.
 *   spacekeyring  — space-wide E2EE keyring. ONE keyring per space; all `enc` nodes
 *                   in the space are encrypted under this single key.

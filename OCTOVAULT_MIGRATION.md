...
- [ ] **Phase 5 — Public directory projection** (server-side): rebuild the public-object directory from `objpub`-collection writes so newly published nodes appear in the global object directory; Python octovault router currently has no projection plugin

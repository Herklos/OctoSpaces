 ...
 * does, so the host's bundler redirects `hash-wasm` to this module on every
 * platform (see the consumer's Metro/bundler config) and the bundle uses a
 * pure-JS Argon2id instead.
 *
 ...
 * React hook wrapper lives in the host app). Onboarding screens use it to show a
 * percentage while the (~30–120 s on Hermes) derivation runs, so the button
 * doesn't look frozen.
 */

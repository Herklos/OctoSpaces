...
import { inviteToSpace, createSpaceInviteLink, joinSpaceByLink, addDeviceToSpaceKeyring, recoverSpaceAccess } from '../../src/spaces/members.js';
import { addCollectionRecipient } from '@drakkar.software/starfish-keyring';
import { addSpaceMember } from '../../src/spaces/registry.js';
import { saveSpaceAccessEntry } from '../../src/sync/space-access-store.js';
import { sealToSelf, unsealFromSelf } from '../../src/sync/account-seal.js';
...
describe('createSpaceInviteLink — keyring ordering and error handling', () => {
  ...
  it('returned link fragment contains the ephemeral private key', async () => {
    ...
    const { fromBase64Url } = await import('../../src/sync/base64.js');
...
describe('recoverSpaceAccess — threads kemPriv/kemPub through unseal → hydrate (Fix C)', () => {
  ...
  it('FIX C: hydrates link access with kemPriv/kemPub when present in sealed blob', async () => {
    const { hydrateSpaceAccessStore } = await import('../../src/sync/space-access-store.js');
  ...
  it('backfill sealToSelf includes kemPriv/kemPub from local link entry', async () => {
    const { localSpaceAccessEntries } = await import('../../src/sync/space-access-store.js');

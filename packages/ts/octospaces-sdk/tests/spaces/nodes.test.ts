...
import { ensureNodeKeyringRecipient } from '../../src/sync/node-keyring.js';
import { addSpaceMember } from '../../src/spaces/registry.js';
import { saveNodeStreamAccessEntry, saveSpaceAccessEntry, saveNodeAccessEntry, saveNodeKeyringAccessEntry } from '../../src/sync/space-access-store.js';
...
describe('createNode', () => {
  ...
  it('pushes the new node to the index (via the space client)', async () => {
    ...
    const { getSpaceClient } = await import('../../src/sync/space-access.js');
...
describe('joinNodeByLink', () => {
  ...
  it('appends a Space entry for the node into the spaces array', async () => {
    const { updateSpacesDoc } = await import('../../src/spaces/registry.js');
